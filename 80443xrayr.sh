...
bash <(curl -ls https://raw.githubusercontent.com/AZZ-vopp/Z_OV/main/xrayrthuong.sh)
    ...
    read -p "nhập ip vps vpn đag cài node 80:" CertDomain1
    echo -e "CertDomain = ${CertDomain1}"

    read -p "Nhập dns trên cloudflare node 443:" CertDomain2
    echo -e "CertDomain = ${CertDomain2}"           
  ...
  -
    ...
    ControllerConfig:
      ...
      CertConfig:
        ...
        CertDomain: "$CertDomain2" # Domain to cert
        CertFile: /etc/XrayR/cert/node1.test.com.cert # Provided if the CertMode is file
        KeyFile: /etc/XrayR/cert/node1.test.com.key
        Provider: cloudflare # DNS cert provider, Get the full support list here: https://go-acme.github.io/lego/dns/
        Email: test@me.com
